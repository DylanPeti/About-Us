...
$postID = $post->ID;
            ...
            $meta_values[] = get_post_meta($postID, 'adhub', true ); //button link
         ...
         //   $video_url = ($_GET['url']) ? $_GET['url'] : $meta_values[0];
           ...
           if(isset($xml_url)) {
            ...
            $img = wp_get_attachment_image_src(get_post_thumbnail_id($postID), $size = 'full'); 
            $thumnanail = wp_get_attachment_thumb_url(get_post_thumbnail_id($postID));
            $thumbnail = get_the_post_thumbnail( $postID, 'homepage-thumb-top'); 

?>
<?php get_header(); ?>

 <div class="video-request-container-offer" style="background: url('<?php echo $img[0]; ?>') no-repeat;">

 <div class="offers-img" style="background: url('<?php  echo $img[0]; ?>') no-repeat;"></div>
      <div class="left-section single-offers-section">
         ...
         <section class="">
          ...
          //   echo do_shortcode("<?php $meta_values[6]"); 
              }
              ?>
             ...
             <?php if(isset($meta_values[7]) && !empty($meta_values[7])) : 
              echo do_shortcode("<?php $meta_values[7]");
              endif;
...
<?php if(empty($meta_values[6]) && empty($meta_values[7])) { ?>
