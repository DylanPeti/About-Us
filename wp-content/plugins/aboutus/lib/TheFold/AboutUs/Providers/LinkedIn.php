...
// define('META_KEY', 'linkedin-profile');   
class LinkedIn extends Provider {
    const META_KEY = 'LinkedIn';
    ...
    static function get_public_activity($sms_id,$token,$user_id, $biz = false)
    {
        ...
        if($profile = get_user_meta( $user_id,'', true )){
    ...
    protected function init_profile()
    {
    	...
    	if($profile = get_user_meta( get_current_user_id(),'', true ))
        {
    ...
    */
    ...
    function setUserStatus( $status )
    {
        ...
        if( is_array( $status ) ){
            ...
            var_dump($this->api->share( 'new', $parameters, $private ));
        }
