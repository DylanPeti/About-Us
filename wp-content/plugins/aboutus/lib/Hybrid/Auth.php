...
class Hybrid_Auth
{
	...
	*/
	...
	//MMM
		...
		//do a quick check if the user is connected in database (MMMMMM)
										

		//check (hack) if url has about in it
		if ($providerId == "Facebook"){	

					$url = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
					if (false !== strpos($url,'/about/')) {
                        $postAuthor = TheFold\AboutUs\get_biz_from_user()->postAuthor;
						$biz = TheFold\AboutUs\get_biz_from_user( get_userdata(  $postAuthor ) );
	...
	*/
	public static function setup( $providerId, $params = NULL )
	{
		...
		if( ! $params ){
			...
			Hybrid_Logger::debug( "Hybrid_Auth::setup( $providerId ), no params given. Trying to get the stored params for this provider.", $params );
		}
