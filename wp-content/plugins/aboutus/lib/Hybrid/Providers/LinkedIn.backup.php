...
class Hybrid_Providers_LinkedIn extends Hybrid_Provider_Model
{
	...
	*/
	function loginBegin()
	{
		...
		if( isset( $response['success'] ) && $response['success'] === TRUE ){
			...
			throw new Exception( "Authentication failed! 1 {$this->providerId} returned an invalid Token.", 5 );
		}
	}
	...
	*/
	function loginFinish()
	{
		...
		if ( ! $oauth_verifier ){
			throw new Exception( "Authentication failed! 2 {$this->providerId} returned an invalid Token.", 5 );
		}
	...
	*/
	function getUserProfile()
	{
		...
		if(strpos($response['linkedin'], 'Expired access token') !== false) {
			$this->logout();
			$hauth = TheFold\AboutUs\get_hauth();
			$service = $hauth->authenticate( 'LinkedIn' );
		}
